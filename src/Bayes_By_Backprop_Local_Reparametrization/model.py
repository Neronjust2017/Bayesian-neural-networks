...
class bayes_linear_LR_1L_homo(nn.Module):
    def __init__(self, input_dim, output_dim, nhid, prior_sig, init_log_noise=0):
        super(bayes_linear_LR_1L_homo, self).__init__()
...
class bayes_linear_LR_1L_hetero(nn.Module):
    def __init__(self, input_dim, output_dim, nhid, prior_sig):
        super(bayes_linear_LR_1L_hetero, self).__init__()
        ...
        self.bfc1 = BayesLinear_local_reparam(input_dim, n_hid, self.prior_sig)
        self.bfc2 = BayesLinear_local_reparam(n_hid, 2 * output_dim, self.prior_sig)
...
class BBP_Bayes_Net_LR_BH_homo(BaseNet):
    """Full network wrapper for Bayes By Backprop nets with methods for training, prediction and weight prunning"""
    def __init__(self, lr=1e-3,  input_dim=13, cuda=True, output_dim=1, batch_size=128, Nbatches=0,nhid=1200, prior_sig=0.1,momentum=0):
        super(BBP_Bayes_Net_LR_BH_homo, self).__init__()
        cprint('y', ' Creating Net!! ')
        self.lr = lr
        self.schedule = None  # [] #[50,200,400,600]
        self.cuda = cuda
    ...
    def create_net(self):
        ...
        self.model = bayes_linear_LR_1L_homo(input_dim=self.input_dim, output_dim=self.output_dim,
                                     nhid=self.nhid, prior_sig=self.prior_sig)
        if self.cuda:
            self.model = self.model.cuda()
...
class BBP_Bayes_Net_LR_BH_hetero(BaseNet):
    """Full network wrapper for Bayes By Backprop nets with methods for training, prediction and weight prunning"""
    def __init__(self, lr=1e-3,  input_dim=13, cuda=True, output_dim=1, batch_size=128, Nbatches=0,nhid=1200, prior_sig=0.1,momentum=0):
        super(BBP_Bayes_Net_LR_BH_hetero, self).__init__()
        cprint('y', ' Creating Net!! ')
        self.lr = lr
        self.schedule = None  # [] #[50,200,400,600]
        self.cuda = cuda
    ...
    def create_net(self):
        ...
        self.model = bayes_linear_LR_1L_hetero(input_dim=self.input_dim, output_dim=self.output_dim,
                                     nhid=self.nhid, prior_sig=self.prior_sig)
        if self.cuda:
            self.model = self.model.cuda()
