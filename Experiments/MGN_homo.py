...
class MGN_Layer(nn.Module):
    def __init__(self, input_dim, output_dim, dropout_prob):
        super(MGN_Layer, self).__init__()
    ...
    def forward(self, x):
        # (1 - self.dropout_prob) * torch.ones(self.weights.shape)
        dropout_mask = torch.normal(mean=torch.ones(self.weights.shape),std=self.dropout_prob/(1-self.dropout_prob)).cuda()

        return torch.mm(x, self.weights * dropout_mask) + self.biases


class MGN_Model(nn.Module):
    def __init__(self, input_dim, output_dim, no_units, init_log_noise):
        super(MGN_Model, self).__init__()
        ...
        # self.mgn = MGN_Layer(no_units, no_units, 0.5)
    ...
    def forward(self, x):
        ...
        # x = self.mgn(x)
        mask = torch.normal(mean=torch.ones(x.shape),
                                    std=prob / (1 - prob)).cuda()
        x = mask * x
...
class MGN_Wrapper:
    def __init__(self, input_dim, output_dim, no_units, learn_rate, batch_size, no_batches, weight_decay,
        ...
        self.network = MGN_Model(input_dim=input_dim, output_dim=output_dim,
...
net = MGN_Wrapper(input_dim=1, output_dim=1, no_units=200, learn_rate=1e-2,
...
plt.savefig('mgn_homo.png', bbox_inches='tight')
