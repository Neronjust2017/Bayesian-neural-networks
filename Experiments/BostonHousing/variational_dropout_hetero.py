...
if __name__ == '__main__':
    ...
    # Paths
    base_dir = './results_hetero/vd_results'
    ...
    for lr in lrs:
        for momentum in momentums:
            for n_sample in n_samples:
                ...
                Hps = 'Lr_' + str(lr) + '_Momentum_' + str(momentum) + '_N_sample_' + str(n_sample)
                ...
                for split in range(int(n_splits)):
                    ...
                    # net dims
                    cprint('c', '\nNetwork:')
                    net = VD_Bayes_Net_BH_hetero(lr=lr, input_dim=inputs, output_dim=outputs, cuda=use_cuda,
                    ...
                    for i in range(epoch, nb_epochs):
                        ...
                        if i % nb_its_dev == 0:
                            ...
                            for j, (x, y) in enumerate(valloader):
                                cost, mse, _, _, _ = net.eval(x, y, samples=T)  # This takes the expected weights to save time, not proper inference
                                cost_dev[i] += cost
                                rmse_dev[i] += mse
                                nb_samples += len(x)
                    ...
                    for j, (x, y) in enumerate(testloader):
                        ...
                        cost, mse, mean, std, noise = net.eval(x, y, samples=T)  # This takes the expected weights to save time, not proper inference
                        if use_cuda:
                            mean = mean.cpu()
                            std = std.cpu()
                            noise = std.cpu()
                    ...
                    store_results(results_file, ['rmse %f PICP %f MPIW %f' % (rmse_test * y_stds, PICP, MPIW) + '\n'])
                ...
                store_results(results_file, ['Overall: \n rmses %f +- %f (stddev) +- %f (std error) PICP %f MPIW %f\n' % (
                    np.mean(rmses), np.std(rmses), np.std(rmses) / math.sqrt(n_splits), np.mean(picps),
                    np.mean(mpiws))])
